...
const GalleryReligious = () => {
  ...
  const fetchImages = async () => {
    try {
      ...
      const filtered = res.data.filter(img => img.category === "Religious Activities");
      setImages(filtered);
    } catch (err) {
  ...
  return (
    <div style={container}>
      <header style={header}>
        <h1 style={title}>{isMarathi ? "📸 धार्मिक उपक्रम गॅलरी" : "📸 Religious Activities Gallery"}</h1>
        <p style={sub}>{isMarathi ? "आरती, महाप्रसाद आणि धार्मिक विधींचे क्षण" : "Moments of daily aarti, prayers, and traditional rituals"}</p>
      </header>
...
export default GalleryReligious;
