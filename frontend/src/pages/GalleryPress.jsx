...
const GalleryPress = () => {
  ...
  const fetchImages = async () => {
    try {
      ...
      const filtered = res.data.filter(img => img.category === "Press Coverage");
      setImages(filtered);
    } catch (err) {
  ...
  return (
    <div style={container}>
      <header style={header}>
        <h1 style={title}>{isMarathi ? "📸 वृत्तपत्र वार्तांकन गॅलरी" : "📸 Press Coverage Gallery"}</h1>
        <p style={sub}>{isMarathi ? "लोकप्रिय वृत्तपत्रांमधील बातम्या आणि प्रसिद्धी पत्रके" : "Newspaper articles, media mentions, and publicity logs"}</p>
      </header>
